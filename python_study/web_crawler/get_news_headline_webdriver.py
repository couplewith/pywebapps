...
def crawler_blog(url, html_selector, headers, DEBUG):
...
def crawler_exec(url_base, html_selector, headers, DEBUG):
    url_range = [1, 500]

    for idx in range(url_range[0], url_range[1], 1):
        url = url_base + "/" + str(idx)
        print(url)
        crawler_blog(url, html_selector, headers, DEBUG)

        sleep(1.0)

def webdriver_crawler_blog(url, html_selector, headers, wdriver, DEBUG):
...
def webdriver_crawler_exec(url_base, html_selector, headers, DEBUG):
    ...
    for idx in range(url_range[0], url_range[1], -1):
        url = url_base + "/" + str(idx)
        print(url)
        webdriver_crawler_blog(url, html_selector, headers, wdriver, DEBUG)
...
match test_mode:
        case 'request_all':
            crawler_exec(url_base, html_selector, headers, DEBUG)
        case 'request_test':
            crawler_blog(url_base + "/" + str(300), html_selector, headers,  DEBUG)
        case 'webdriver_all':
            webdriver_crawler_exec(url_base, html_selector, headers, DEBUG)
        case 'webdriver_test':
            wdriver = webdriver.Chrome()
            webdriver_crawler_blog(url_base + "/" + str(300), html_selector, headers, wdriver, DEBUG)
        case other:
